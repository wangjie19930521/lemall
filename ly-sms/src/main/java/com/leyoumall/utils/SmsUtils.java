...
public class SmsUtils {
    ...
    public SendSmsResponse sendSms(String phone, String code, String signName, String template) {
        //todo 限流，发送成功根据手机号，设置过期时间  redis   每次进入先查询redis
        try {
            //可自助调整超时时间
            ...
            if (!"OK".equals(sendSmsResponse.getCode())) {
                log.error("短信发送失败，电话：{}原因：{}", phone, sendSmsResponse.getMessage());
            }
            return sendSmsResponse;
        }catch(Exception e){
            log.error("短信发送失败，电话：{}原因：{}", phone, e);
            return null;
        }
    }
